...
// placeable LIGHTS (own list — counts against the per-map light cap)
export const LIGHT_KINDS = { point: { label: 'Point', dist: 16, intensity: 2.0 }, spot: { label: 'Spot', dist: 20, intensity: 2.5 } };
export const MAX_CUSTOM_LIGHTS = 10;  // editor enforces + warns at this cap (perf)
...
export function customToMapData(custom) {
  ...
  for (const o of (c.objects || [])) {
    if (o.kind === 'spawn') spawns.push({ pos: { x: o.x || 0, y: o.y || 0, z: o.z || 0 }, yaw: o.yaw || 0, team: o.team });
      ...
      pickups.push({
        ...
        weapon: o.weapon, amount: o.amount, powerup: o.powerup,
  ...
  const hazards = [{ id: 'cm_void', type: 'void', min: { x: -400, y: floorY, z: -400 }, max: { x: 400, y: minY - 6, z: 400 }, dps: 200 }];
